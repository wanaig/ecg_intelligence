...
CREATE TABLE `sys_ecg_measure_stat` (
  ...
  `ward_id` INT NULL COMMENT '病区ID',
  `equipment_type` VARCHAR(30) NULL COMMENT '设备类型',
  `total_measure` INT DEFAULT 0 NULL COMMENT '总测量次数',
  `normal_measure` INT DEFAULT 0 NULL COMMENT '正常次数',
  `abnormal_measure` INT DEFAULT 0 NULL COMMENT '异常次数',
  `stat_cycle` VARCHAR(20) NULL COMMENT '统计周期'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='心电测量统计报表';

CREATE TABLE `sys_ecg_report_stat` (
  `report_stat_id` INT AUTO_INCREMENT PRIMARY KEY COMMENT '报告统计ID',
  `stat_time` DATETIME NULL COMMENT '统计时间',
  `ward_id` INT NULL COMMENT '病区ID',
  `total_report` INT DEFAULT 0 NULL COMMENT '总报告数',
  `draft_report` INT DEFAULT 0 NULL COMMENT '草稿数',
  `pending_review_report` INT DEFAULT 0 NULL COMMENT '待审核数',
  `approved_report` INT DEFAULT 0 NULL COMMENT '已审核数',
  `archived_report` INT DEFAULT 0 NULL COMMENT '已归档数',
  `avg_review_time` DECIMAL(10, 2) DEFAULT 0.00 NULL COMMENT '平均审核时长(分钟)',
  `stat_cycle` VARCHAR(20) NULL COMMENT '统计周期'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='心电图报告统计报表';

CREATE TABLE `sys_ecg_warning_stat` (
  `warning_stat_id` INT AUTO_INCREMENT PRIMARY KEY COMMENT '预警统计ID',
  `stat_time` DATETIME NULL COMMENT '统计时间',
  `ward_id` INT NULL COMMENT '病区ID',
  `warning_level` VARCHAR(20) NULL COMMENT '预警级别',
  `warning_type` VARCHAR(30) NULL COMMENT '预警类型',
  `total_warning` INT DEFAULT 0 NULL COMMENT '总预警次数',
  `handled_warning` INT DEFAULT 0 NULL COMMENT '已处理次数',
  `unhandled_warning` INT DEFAULT 0 NULL COMMENT '未处理次数',
  `avg_handle_time` DECIMAL(10, 2) DEFAULT 0.00 NULL COMMENT '平均处理时长(分钟)',
  `stat_cycle` VARCHAR(20) NULL COMMENT '统计周期'
