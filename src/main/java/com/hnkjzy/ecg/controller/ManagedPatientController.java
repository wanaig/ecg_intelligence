...
@RestController
@RequestMapping("/api/managed-patients")
/**
 * 类说明：ManagedPatientController。
 * 业务定位：负责接收前端菜单请求、参数解析并返回统一 REST 响应。
 * 说明补充：该类中的字段、方法和返回值均遵循统一命名与结构规范。
 */
public class ManagedPatientController {

    private final EcgBusinessService ecgBusinessService;

    public ManagedPatientController(EcgBusinessService ecgBusinessService) {
     ...
     */
    public ApiResponse<List<PatientManageView>> list() {
        return ApiResponse.success(ecgBusinessService.listManagedPatients());
    }
}
